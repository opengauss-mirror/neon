...
pub const fn clogpage_precedes(page1: u32, page2: u32) -> bool {
    ...
    transaction_id_precedes(xid1 as TransactionId, xid2 as TransactionId)
        && transaction_id_precedes(xid1 as TransactionId, (xid2 + pg_constants::CLOG_XACTS_PER_PAGE - 1) as TransactionId)
}
...
pub fn mx_offset_to_flags_offset(xid: MultiXactId) -> usize {
    ((xid / pg_constants::MULTIXACT_MEMBERS_PER_MEMBERGROUP as u64)
        % pg_constants::MULTIXACT_MEMBERGROUPS_PER_PAGE as u64
        * pg_constants::MULTIXACT_MEMBERGROUP_SIZE as u64) as usize
}
...
mod tests {
    ...
    fn test_multixid_calc() {
        ...
        // program.
        // Note: openGauss uses MultiXactId as u64, so we need to use u64 literals
        assert_eq!(mx_offset_to_member_segment(0), 0);
        assert_eq!(mx_offset_to_member_page(0), 0);
        assert_eq!(mx_offset_to_flags_offset(0u64), 0);
        assert_eq!(mx_offset_to_flags_bitshift(0u64), 0);
        assert_eq!(mx_offset_to_member_offset(0u64), 4);
        assert_eq!(mx_offset_to_member_segment(1), 0);
        assert_eq!(mx_offset_to_member_page(1), 0);
        assert_eq!(mx_offset_to_flags_offset(1u64), 0);
        assert_eq!(mx_offset_to_flags_bitshift(1u64), 8);
        assert_eq!(mx_offset_to_member_offset(1u64), 8);
        assert_eq!(mx_offset_to_member_segment(123456789), 2358);
        assert_eq!(mx_offset_to_member_page(123456789), 75462);
        assert_eq!(mx_offset_to_flags_offset(123456789u64), 4780);
        assert_eq!(mx_offset_to_flags_bitshift(123456789u64), 8);
        assert_eq!(mx_offset_to_member_offset(123456789u64), 4788);
        assert_eq!(mx_offset_to_member_segment(u32::MAX - 1), 82040);
        assert_eq!(mx_offset_to_member_page(u32::MAX - 1), 2625285);
        assert_eq!(mx_offset_to_flags_offset((u32::MAX - 1) as u64), 5160);
        assert_eq!(mx_offset_to_flags_bitshift((u32::MAX - 1) as u64), 16);
        assert_eq!(mx_offset_to_member_offset((u32::MAX - 1) as u64), 5172);
        assert_eq!(mx_offset_to_member_segment(u32::MAX), 82040);
        assert_eq!(mx_offset_to_member_page(u32::MAX), 2625285);
        assert_eq!(mx_offset_to_flags_offset(u32::MAX as u64), 5160);
        assert_eq!(mx_offset_to_flags_bitshift(u32::MAX as u64), 24);
        assert_eq!(mx_offset_to_member_offset(u32::MAX as u64), 5176);
    }
}
