...
///
/// For CLOG files, we need to mark all IN_PROGRESS transactions as COMMITTED.
/// This is necessary because during openGauss initdb (bootstrap mode), transactions
/// don't write WAL records for their commits. The clog file created by initdb
/// contains all zeros (IN_PROGRESS), but all bootstrap transactions should be
