...
extern "C" void InitialzeCitusWaitEventSet()
{
 ...
 * lifetime.
 */
extern "C" WaitEventSet* CreateWaitEventSet(ResourceOwner resowner, int nevents)
{
    WaitEventSet* set;
    char* data;
    Size sz = 0;
 ...
 */
extern "C" void FreeWaitEventSet(WaitEventSet* set)
{
 ...
 */
extern "C" void FreeWaitEventSetAfterFork(WaitEventSet* set)
{
    set->m_IOMultiplexor->Free();
 ...
 * events.
 */
extern "C" int AddWaitEventToSet(WaitEventSet* set, uint32 events, pgsocket fd, Latch* latch,
                      void* user_data)
{
    WaitEvent* event;
 ...
 */
extern "C" void ModifyWaitEvent(WaitEventSet* set, int pos, uint32 events, Latch* latch)
{
    WaitEvent* event;
 ...
 */
extern "C" int WaitEventSetWait(WaitEventSet* set, long timeout, WaitEvent* occurred_events,
                     int nevents, uint32 wait_event_info)
{
    ...
    while (returned_events == 0) {
         ...
         * having to drain it every time NeonWaitLatchOrSocket() is used. Should
 ...
 */
extern "C" bool WaitEventSetCanReportClosed(void)
{
 ...
 */
extern "C" int GetNumRegisteredWaitEvents(WaitEventSet* set)
{
    return set->nevents;
}
 ...
 */
extern "C" int NeonWaitLatchOrSocket(Latch* latch, int wakeEvents, pgsocket sock, long timeout,
                      uint32 wait_event_info)
{
    ...
    /*
     * openGauss difference:
     * In a number of background processes (e.g. WLMworker), Neon can be
     * loaded and NeonWaitLatchOrSocket() called before CurrentResourceOwner
     * is initialized. Passing a NULL owner to CreateWaitEventSet would lead
     * to a crash inside CreateWaitEventSet.
     *
     * In those cases, fall back to the server's own WaitLatchOrSocket()
     * implementation, ignoring wait_event_info. This preserves correct latch
     * / socket waiting semantics without relying on Neon-specific wait
     * infrastructure.
     */
    if (t_thrd.utils_cxt.CurrentResourceOwner == NULL)
        return WaitLatchOrSocket(latch, wakeEvents, sock, timeout);

    set = CreateWaitEventSet(t_thrd.utils_cxt.CurrentResourceOwner, 3);
